...
class LouerController {
    ...
    public function create(LouerModel $louer) {
        $stmt = $this->pdo->prepare("
            INSERT INTO louer (user_id, produit, nom, prenom, date_location, heure_debut, heure_fin, telephone, carte_identite, created_at, statut_location)
            VALUES (:user_id, :produit, :nom, :prenom, :date_location, :heure_debut, :heure_fin, :telephone, :carte_identite, :created_at, :statut_location)
        ");
    ...
    public function validerLocation() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                if (
                    isset($_POST['produit'], $_POST['nom'], $_POST['prenom'],
                          $_POST['telephone'], $_POST['carte_identite'],
                          $_POST['heure_debut'], $_POST['heure_fin'], $_POST['produit_id'])
                ) {
                    ...
                    // Remplacer par $_SESSION['user_id'] si la session utilisateur est active
                    $utilisateurId = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 15;
                    ...
                    $stmt = $this->pdo->prepare("INSERT INTO louer (
                        ...
                        telephone, carte_identite, user_id, created_at, statut_location
                    ) VALUES (
                        :produit, :nom, :prenom, :date_location, :heure_debut, :heure_fin, 
                        :telephone, :carte_identite, :user_id, NOW(), 'en_attente'
                    )");
                    ...
                    $stmt->bindParam(':user_id', $utilisateurId);
