...
try {
    ...
    header('Location: http://localhost:8000/view/front%20office/commande_success.php');
    exit;
} catch (\Stripe\Exception\ApiErrorException $e) {
    error_log('Stripe Error: ' . $e->getMessage());
    header('Location: http://localhost:8000/view/front%20office/panier.php?error=' . urlencode('Erreur de paiement Stripe: ' . $e->getMessage()));
    exit;
} catch (Exception $e) {
    error_log('General Error: ' . $e->getMessage());
    header('Location: http://localhost:8000/view/front%20office/panier.php?error=' . urlencode('Erreur: ' . $e->getMessage()));
    exit;
}