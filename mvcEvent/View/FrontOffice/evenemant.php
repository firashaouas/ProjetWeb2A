...
$user_id = 3; // Set to 4 for testing
...
if ($user_id !== null && is_int($user_id) && $user_id > 0) {
    try {
        $db = new PDO('mysql:host=localhost;dbname=baseprojet;charset=utf8mb4', 'root', '');
...
// Fetch recommendations (using hardcoded user_id)
$python_script = 'C:/xampp/htdocs/projetWeb/mvcEvent/recommend_events.py';
$command = escapeshellcmd("python $python_script " . $user_id);
$output = shell_exec($command);
$recommendations_data = json_decode($output, true);
$recommendations = $recommendations_data['success'] ? $recommendations_data['recommendations'] : [];
?>
...
<body>
    ...
    <div class="container">
    <h2 class="subtitle"><?php echo $is_new_user ? 'Découvrir' : 'Recommandations pour vous'; ?></h2>
    <div class="exclusives-wrapper">
        <div class="exclusives">
            ...
            foreach ($recommendations as $rec):
                $display_style = ($count >= $max_display) ? 'style="display: none;"' : '';
            ?>
                <div class="exclusive-item" data-event-id="<?php echo $rec['event']['id']; ?>" <?php echo $display_style; ?>>
                    <img src="<?php echo htmlspecialchars($rec['image']); ?>" alt="<?php echo htmlspecialchars($rec['event']['name']); ?>">
                    <h3><?php echo htmlspecialchars($rec['event']['name']); ?></h3>
                    <p><?php echo htmlspecialchars($rec['event']['price']); ?> DT</p>
                    <a href="reservation.php?event_id=<?php echo $rec['event']['id']; ?>" class="register-btn">Réserver</a>
                </div>
            <?php
                $count++;
            endforeach;
        ...
        <?php if (count($recommendations) > $max_display): ?>
   ...
   const USER_ID = <?php echo json_encode($user_id); ?>; // Will be 4
    ...
    window.openEditModal = async function(eventId) {
        try {
            ...
            const [eventResponse, seatsResponse, reservationResponse] = await Promise.all([
                fetch(`${API_BASE_URL}?action=get_event&event_id=${eventId}&user_id=${USER_ID}`, { credentials: 'same-origin' }),
                fetch(`${API_BASE_URL}?action=get_seats&event_id=${eventId}&user_id=${USER_ID}`, { credentials: 'same-origin' }),
                fetch(`${API_BASE_URL}?action=get_reservation&event_id=${eventId}&user_id=${USER_ID}`, { credentials: 'same-origin' })
            ]);
    ...
    function trackEventClick(eventId) {
        fetch('/projetWeb/mvcEvent/track_click.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                user_id: USER_ID,
