...
switch ($action) {
    ...
    case 'statistics':
        ...
        break;
    case 'daily_activity':
        $data = $activityController->daily_activity();
        break;
    case 'history':
        ...
        function selectDate(year, month, day, cell) {
          ...
          fetch(`dashboard.php?action=getActivitiesBySelectedDate&date=${yearStr}-${monthStr}-${dayStr}`)
            .then(async response => {
              ...
              if (data.success) {
                const hasActivities = (data.activities && data.activities.length > 0) || (data.enterpriseActivities && data.enterpriseActivities.length > 0);
                if (hasActivities) {
                  let html = '';
                  if (data.activities && data.activities.length > 0) {
                    data.activities.forEach(activity => {
                      html += `<div class=\"date-activity-item\"><h4>${activity.name}</h4><p>Localisation: ${activity.location}</p><p>Prix: ${activity.price} DT</p><span class=\"activity-type-badge badge-regular\">Activité régulière</span></div>`;
                    });
                  }
                  if (data.enterpriseActivities && data.enterpriseActivities.length > 0) {
                    data.enterpriseActivities.forEach(activity => {
                      html += `<div class=\"date-activity-item date-enterprise-item\"><h4>${activity.name}</h4><p>Catégorie: ${activity.category}</p><p>Prix: ${activity.price} DT</p><span class=\"activity-type-badge badge-enterprise\">Activité d'entreprise</span></div>`;
                    });
                  }
            ...
            .catch(error => {
              document.getElementById('date-activities-container').innerHTML = `<p class=\"no-activities-message\">Erreur JS ou réseau : ${error.message}</p>`;
            });
        }
        ...
        <div class="category-content <?php echo $is_active; ?>" id="<?php echo $category_key; ?>-content">
          ...
          <div class="activities-grid">
            ...
            <div class="activity-card">
              <div class="activity-image">
                    <?php if (!empty($activity['image'])): ?>
                      <img src="../front office/<?php echo htmlspecialchars($activity['image']); ?>" alt="<?php echo htmlspecialchars($activity['name']); ?>">
                    <?php else: ?>
                      <div class="no-image">Pas d'image disponible</div>
                    <?php endif; ?>
              </div>
    ...
    <?php elseif ($section === 'edit_enterprise'): ?>
      ...
      <div class="enterprise-edit-section">
        ...
        <div class="add-activity-form">
          ...
          // Vérifier si l'ID et la catégorie sont présents
        ...
        <div class="reservations-table-wrapper">
          <table class="data-table" id="reservations-table">
            ...
            <tbody>
                ...
                <tr data-status="<?php echo $reservation['payment_status']; ?>" data-date="<?php echo $formattedDate; ?>">
                  ...
                  <td>
                    <?php if ($reservation['payment_status'] == 'pending'): ?>
                      <a href="process_reservation.php?action=confirm&id=<?php echo $reservation['id']; ?>" class="btn-confirm" onclick="return confirm('Êtes-vous sûr de vouloir confirmer cette réservation ?')">Confirmer</a>
                    <?php endif; ?>
                    <?php if ($reservation['payment_status'] != 'cancelled'): ?>
                      <a href="process_reservation.php?action=cancel&id=<?php echo $reservation['id']; ?>" class="btn-cancel" onclick="return confirm('Êtes-vous sûr de vouloir annuler cette réservation ?')">Annuler</a>
                    <?php endif; ?>
                  </td>
                </tr>
              <?php 
              ...
              ?>
                <tr>
                  <td colspan="9" class="empty-table">Aucune réservation trouvée</td>
                </tr>
              <?php endif; ?>
            </tbody>
          </table>
